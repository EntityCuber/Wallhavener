...
def set_wall_on_linux():  # need more methods for all linux
    set_wall_method_nitrogen()


def set_wall_method_nitrogen():
    statusVar.set('setting wallpaper using nitrogen')
    global wallpath
    if (saveVar.get()):
        os.system(f'nitrogen {wallpath} --set-zoom-fill --save')
        statusVar.set(f'{wallpath} wallpaper set and saved')
    else:
        os.system('nitrogen .wallpaper.jpg --set-zoom-fill --save')
        statusVar.set('wallpaper set')
        # print('wallpaper applied')
